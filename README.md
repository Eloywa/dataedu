...
Песочница PostgreSQL в браузере (PGlite): `/trainer/`. Три набора данных на
выбор — университет, магазин, библиотека; университетский совпадает с заготовкой
практических заданий. База каждого студента хранится в IndexedDB его браузера и
переживает перезагрузку; «Сбросить базу» возвращает набор к исходному виду.
...
(ER-схема по внешним ключам, таблицы перетаскиваются), **«Ссылка»** (запрос в
адресной строке) и выгрузка результата в CSV.
