// Наборы данных для песочницы: три учебных и один пустой.
//
// Учебных именно три, и это не разнообразие ради разнообразия. На одном наборе
// студент запоминает не приём, а конкретные имена: «соединение — это students и
// groups». Тот же JOIN на магазине и библиотеке заставляет увидеть за именами
...
//
// Четвёртый набор — пустой. Создавать свои таблицы можно было и раньше, но они
// лежали вперемешку с учебными и пропадали при сбросе; отдельный режим даёт
// собственному проекту своё место и делает эту возможность видимой.
//
// У учебных наборов есть `bulk` — вставка, раздувающая набор до двухсот тысяч
// строк. Без неё разговор об индексах беспредметен: на двенадцати строках
// планировщик индекс не возьмёт, и правильно сделает. У пустого набора вместо
// него `starter` — заготовка в редакторе.
...
FROM generate_series(1, 200000) AS g;`,
  ...
  blank: {
    title: "Своя база",
    note: "таблицы создаёте сами",
    // Пустой набор — не «отсутствие набора», а полноценный режим: место под
    // собственный проект студента. Учебные данные его больше не затирают, а
    // «Сбросить базу» здесь очищает схему, а не возвращает чужие таблицы.
    sql: "",
    emptyHint: "Пока пусто. Напишите CREATE TABLE и нажмите «Выполнить» — таблица появится здесь.",
    // Начальный текст в редакторе. Пустое поле на пустой базе не подсказывает
    // ничего, а с чего начинать проектирование — как раз то, чему учат.
    starter: `-- Своя база: спроектируйте схему сами.
-- Пример: справочник и журнал, связанные внешним ключом.
...
CREATE TABLE книги (
  ...
  жанр_id  integer REFERENCES жанры(id),
  год      integer CHECK (год > 1400)
);

INSERT INTO жанры (имя) VALUES ('Фантастика'), ('История');
INSERT INTO книги (название, жанр_id, год) VALUES
  ('Пикник на обочине', 1, 1972),
  ('Соляной путь', 2, 2015);

SELECT к.название, ж.имя AS жанр, к.год
FROM книги к JOIN жанры ж ON ж.id = к.жанр_id;`,
  },
};
